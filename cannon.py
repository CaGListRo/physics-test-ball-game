...
class Cannon:
    def __init__(self, x: int, y: int, width: int, height: int) -> None:
        ...
        self.pullball = PullBall(x=(self.x - self.width / 2 - self.pullball_radius), y=self.y, radius=int(self.height * 0.45), max_pull_length=int(width * 0.8))
        self.max_pull_length: int = int(width * 0.8)

    def update(self) -> None:
        self.pullball.check_pullball_action(cannon_x=self.x, cannon_y=self.y)
